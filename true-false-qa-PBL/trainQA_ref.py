...
from utlis.prompt import get_examples,get_rel_examples
from typing import *
...
def main(args):
    ...
    #################################################
    if args.use_law:
        trainDataset = get_rel_examples(args.train_data, args.law_file)
        validDataset = get_rel_examples(args.test_data, args.law_file)
    else:
    ...
    '''
    if args.use_law:
        text_template = '根據 {"placeholder":"text_b"}，請問 {"placeholder":"text_a"}，是否正確{"mask"}。'
    else:
        text_template = '{"placeholder":"text_a"}，上述為{"mask"}'
